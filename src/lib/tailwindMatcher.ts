// Default Tailwind CSS color palette (subset of most used)
const TAILWIND_COLORS: Record<string, string> = {
  ...
  "indigo-50":"#eef2ff","indigo-100":"#e0e7ff","indigo-200":"#c7d2fe","indigo-300":"#a5b4fc","indigo-400":"#818cf8","indigo-500":"#6366f1","indigo-600":"#4f46e5","indigo-700":"#4338ca","indigo-800":"#3730a3","indigo-900":"#312e81","indigo-950":"#1e1b4e",
...
export function findClosestTailwind(hex: string): TailwindMatch[] {
  const inputRgb = hexToRgb(hex.replace("#", "").length === 3
    ? hex.replace("#", "").split("").map(c => c+c).join("")
    : hex);

  const results = Object.entries(TAILWIND_COLORS)
    .map(([name, twHex]) => ({
      name,
      hex: twHex,
      distance: colorDistance(inputRgb, hexToRgb(twHex)),
    }))
...
export function getAllTailwindColors() {
